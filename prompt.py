...
3.角色的描述需要尽可能详细，主要着概括其外貌特征，重点写清楚角色的*大致年龄*、*性别*、*国籍*、*突出外貌特征*以及*着装*等，另外可简要概括其性格特点、居住环境以及其余能够影响角色形象的信息，但不要过多描述其余无关信息。
...
5. 角色形象显著变化用 <person> 标签标注角色的变化，注意这里的标签内容有两个部分，其一是角色的唯一姓名，其二是角色状态的一个简短标签，如 <person>张三 伤心</person>，表示张三需要做出伤心的表情，<person>张三 手部受伤</person>说明张三的手部受伤。
6. 请注意角色的姓名是唯一的，你可以使用的角色姓名仅限下方列表：
