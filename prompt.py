...
4. 如果小说中出现了比较大的场景切换，例如人物登上交通工具、进入某个商店或者突然下雨等，使用 <scene> 标签详细描写出新的场景，但要注意场景切换的自然过渡。
   • 场景内容需要尽量避免出现人物信息，例如人物登上交通工具，那么<scene>标签中应当描写登上的交通工具是什么，内部场景是什么样子的，而不是描写某某人登上车之类的
   • 内容要尽量详细，包括场景的可能时代、国家、地点名称、主要地理特征、当时的天气等等非常多的场景标签，你要从故事中把场景信息提取出来
...
由于小说一般非常长，因此我对小说内容进行了切分，你需要查看下面提供的上一段脚本，在上面的小说原文中接着它的内容继续编写脚本。
注意你不需要重复上一段脚本的任何内容（因为我要进行组合），直接输出新的一段小说原文转化的脚本即可。
上一段脚本为：
{previous_script}
你紧接着应该输出的脚本为：
"""